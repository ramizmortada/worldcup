...
const getMatchTimestamp = (m: Match) => {
  if (m.apiDetails?.date) {
    return new Date(m.apiDetails.date).getTime();
  }
  try {
    const str = m.date.split('-')[0].trim();
    return new Date(str.includes('2026') ? str : str + ', 2026').getTime();
  } catch {
    return 0;
  }
};

const sortMatchesByDate = (a: Match, b: Match, asc: boolean) => {
...
export default function TournamentBracket({ appMode = 'play' }: { appMode?: 'live' | 'play' }) {
  ...
  const fetchLiveMatches = (baseMatches: Match[], fallback: boolean = false, mode = tournamentMode) => {
    setIsFetching(true);
    fetch('/api/wc2026-played-matches', { cache: 'no-store' })
      .then(res => {
        if (!res.ok) throw new Error('API response error');
        return res.json();
      })
      .then((data: { matches: { id?: number; teamAId?: string; teamBId?: string; scoreA: number; scoreB: number; winnerHome?: boolean; winnerAway?: boolean; apiDetails?: MatchApiDetails }[] }) => {
        const apiScores = data.matches || [];
        const groupPlayedIds = apiScores.map(a => a.id).filter(id => id !== undefined) as number[];
        
        const merged = baseMatches.map(m => {
          const apiMatch = apiScores.find(a => a.id !== undefined && a.id === m.id);
          if (apiMatch) {
            // Overwrite with accurate score data from API if available
            const updated = { ...m, apiDetails: apiMatch.apiDetails };
            if (apiMatch.scoreA !== undefined && apiMatch.scoreB !== undefined) {
              updated.scoreA = apiMatch.scoreA;
              updated.scoreB = apiMatch.scoreB;
            }
            
            if (apiMatch.teamAId) updated.teamAId = apiMatch.teamAId;
            if (apiMatch.teamBId) updated.teamBId = apiMatch.teamBId;
            
            // Auto-propagate winner if group match and scores exist
        ...
        // Run calculation sequence to propagate knockout teams based on group stages
        let recalculated = propagateMatches(merged, mode);

        // Second pass: apply live scores for knockout matches based on resolved team IDs
        let updatedKnockouts = false;
        recalculated = recalculated.map(m => {
          if (m.stage !== 'group' && !isPlaceholder(m.teamAId) && !isPlaceholder(m.teamBId)) {
            const apiMatch = apiScores.find(a => 
              a.id === undefined && 
              ((a.teamAId === m.teamAId && a.teamBId === m.teamBId) || (a.teamAId === m.teamBId && a.teamBId === m.teamAId))
            );
            if (apiMatch) {
                const isFlipped = apiMatch.teamAId === m.teamBId;
                const updated = { ...m, apiDetails: apiMatch.apiDetails };
                
                if (apiMatch.scoreA !== undefined && apiMatch.scoreB !== undefined) {
                  updatedKnockouts = true;
                  const finalScoreA = isFlipped ? apiMatch.scoreB : apiMatch.scoreA;
                  const finalScoreB = isFlipped ? apiMatch.scoreA : apiMatch.scoreB;
                  
                  updated.scoreA = finalScoreA;
                  updated.scoreB = finalScoreB;
                  ...
                  if (finalScoreA > finalScoreB) {
                    ...
                    // Determine winner from penalty if available via ESPN winner flag
                    const winnerHome = apiMatch.winnerHome;
                    const winnerAway = apiMatch.winnerAway;
                    if (winnerHome) {
                      updated.winnerId = isFlipped ? updated.teamBId : updated.teamAId;
                      updated.penaltiesA = isFlipped ? 4 : 5;
                      updated.penaltiesB = isFlipped ? 5 : 4;
                    } else if (winnerAway) {
                      updated.winnerId = isFlipped ? updated.teamAId : updated.teamBId;
                      updated.penaltiesA = isFlipped ? 5 : 4;
                      updated.penaltiesB = isFlipped ? 4 : 5;
                    }
                  }
                }
                return updated;
        ...
        if (updatedKnockouts) {
          recalculated = propagateMatches(recalculated, mode);
        }

        const knockoutPlayedIds = recalculated.filter(m => m.stage !== 'group' && m.scoreA !== undefined && m.scoreB !== undefined).map(m => m.id);
        setPlayedMatchIds([...groupPlayedIds, ...knockoutPlayedIds]);
  ...
  useEffect(() => {
    let initialMatches = [...generateGroupMatches(), ...KNOCKOUT_TEMPLATES];
      ...
      <div key={m.id} className="relative group cursor-pointer" onClick={() => setSelectedMatchId(m.id)}>
        <div className="flex items-center justify-between py-1.5 px-2 hover:bg-slate-800/30 rounded transition-colors group-hover:bg-slate-800/50">
          ...
          <div className="flex-1 flex items-center justify-end gap-1.5">
            ...
            <Input
              type="text"
              readOnly={isPlayed || tournamentMode === 'performance' || appMode === 'live'}
  ...
  const renderKnockoutCard = (m: Match | undefined) => {
    ...
    return (
      <Card key={m.id} className="relative overflow-visible w-full shrink-0 bg-slate-950/20 border border-slate-800/70 transition-colors p-1.5 flex flex-col justify-center">
        ...
        <div className="space-y-1">
          {/* Team A */}
          <div onClick={() => appMode !== 'live' && !isPlaceholder(m.teamAId) && setKnockoutWinner(m.id, m.teamAId)} className={`flex items-center justify-between h-6 py-0.5 px-1.5 rounded-md transition-colors cursor-pointer hover:bg-slate-800/60 ${
          ...
          {/* Team B */}
          <div onClick={() => appMode !== 'live' && !isPlaceholder(m.teamBId) && setKnockoutWinner(m.id, m.teamBId)} className={`flex items-center justify-between h-6 py-0.5 px-1.5 rounded-md transition-colors cursor-pointer hover:bg-slate-800/60 ${
    ...
    <div className="h-screen w-screen bg-slate-950 text-slate-100 flex flex-col overflow-hidden font-sans select-none">
      ...
      <header className="h-14 bg-slate-900/60 border-b border-slate-800/80 px-6 flex items-center justify-center gap-6 shrink-0 backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-2">
          {appMode === 'live' ? (
            <Link href="/play">
              <Button size="xs" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-3 py-1.5 text-xs shadow-md shadow-indigo-600/15">
                Play Simulator
              </Button>
            </Link>
          ) : (
            <>
              <Link href="/">
                <Button size="xs" variant="outline" className="border-slate-800 text-slate-400 hover:bg-slate-900 hover:text-white mr-2">
                  Live Rankings
                </Button>
              </Link>
              <Button
      ...
      <ScrollArea className="flex-1 min-h-0 min-w-0 p-1 md:p-2 bg-gradient-to-br from-slate-950 via-slate-900/40 to-slate-950">
           {activeTab === 'bracket' ? (
             ...
             <div className="w-full max-w-6xl mx-auto space-y-6 pt-6 pb-32 px-4">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 {/* Upcoming */}
                 <div className="flex flex-col space-y-4">
                   <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center justify-center gap-2 border-b border-slate-800 pb-2">
                     Upcoming
                   </h2>
                   <div className="flex flex-col gap-2">
                     {[...populatedMatches].filter(m => m.scoreA === undefined || m.scoreB === undefined).sort((a, b) => sortMatchesByDate(a, b, true)).map(m => {
                       const teamA = getTeam(m.teamAId);
                       const teamB = getTeam(m.teamBId);
                       return (
                         <div key={m.id} className="flex items-center justify-between bg-slate-900/60 p-2 rounded border border-slate-800 hover:border-slate-700 cursor-pointer transition-colors" onClick={() => setSelectedMatchId(m.id)}>
                           <div className="flex items-center gap-2 w-1/3 justify-end">
                             <span className="font-semibold text-xs text-slate-300 truncate" title={teamA.name}>{teamA.name}</span>
                             <TeamFlag team={teamA} />
                           </div>
                           <div className="flex flex-col items-center justify-center w-1/3">
                             <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">{m.stage === 'group' ? `Group ${m.group}` : m.stage}</span>
                             <div className="text-sm font-bold text-slate-500 tabular-nums mt-0.5">vs</div>
                             <span className="text-[9px] text-slate-400 mt-0.5 whitespace-nowrap">
                               {m.apiDetails?.date ? new Date(m.apiDetails.date).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : m.date}
                             </span>
                           </div>
                           <div className="flex items-center gap-2 w-1/3 justify-start">
                             <TeamFlag team={teamB} />
                             <span className="font-semibold text-xs text-slate-300 truncate" title={teamB.name}>{teamB.name}</span>
                           </div>
                         </div>
                       );
                     })}
                 ...
                 <div className="flex flex-col space-y-4">
                   ...
                   <div className="flex flex-col gap-2">
                     {[...populatedMatches].filter(m => m.scoreA !== undefined && m.scoreB !== undefined).sort((a, b) => sortMatchesByDate(a, b, false)).map(m => {
                       const teamA = getTeam(m.teamAId);
                       const teamB = getTeam(m.teamBId);
                       return (
                         <div key={m.id} className="flex items-center justify-between bg-slate-900/60 p-2 rounded border border-slate-800 hover:border-slate-700 cursor-pointer transition-colors" onClick={() => setSelectedMatchId(m.id)}>
                           <div className="flex items-center gap-2 w-1/3 justify-end">
                             <span className={`font-semibold text-xs truncate ${m.winnerId === m.teamAId ? 'text-emerald-400' : 'text-slate-400'}`} title={teamA.name}>{teamA.name}</span>
                             <TeamFlag team={teamA} />
                           </div>
                           <div className="flex flex-col items-center justify-center w-1/3">
                             <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">{m.stage === 'group' ? `Group ${m.group}` : m.stage}</span>
                             <div className="text-sm font-black text-emerald-400 tabular-nums bg-emerald-950/30 px-2 py-0.5 rounded mt-0.5">
                               {m.scoreA} : {m.scoreB}
                             </div>
                             {m.penaltiesA !== undefined && m.penaltiesB !== undefined && (
                               <span className="text-[9px] font-bold text-amber-500 mt-0.5">({m.penaltiesA}-{m.penaltiesB} pens)</span>
                             )}
                           </div>
                           <div className="flex items-center gap-2 w-1/3 justify-start">
                             <TeamFlag team={teamB} />
                             <span className={`font-semibold text-xs truncate ${m.winnerId === m.teamBId ? 'text-emerald-400' : 'text-slate-400'}`} title={teamB.name}>{teamB.name}</span>
                           </div>
                         </div>
                       );
                     })}
